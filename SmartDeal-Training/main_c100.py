...
from utils_data import CIFAR10, CIFAR100
...
if args.task == 'adapt':
    ...
    plabels = np.random.permutation(100).tolist()
    used_samples = None
    # used_labels = plabels[:50] if args.source else plabels[50:]
    used_labels = plabels[:90] if args.source else plabels[90:]
...
trainset = CIFAR100(root='./data', used_labels=used_labels, used_samples=used_samples,
                    train=True, download=True, transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=2)

testset = CIFAR100(root='./data', used_labels=used_labels, used_samples=None,
...
if args.exp_dir is None:
    arch = 'CIFAR-100-' + args.arch
    arch += '-qes' if args.quant_each_step else ''
    arch += '-thres{}'.format(args.threshold) if 'SEMask' in args.arch else ''
    ...
    exp_dir = os.path.join(
        'results',
        '{}_{}_{}_{}'.format(arch, args.seed, task, 'source' if args.source else 'target')
    )
else:
...
if args.validate and args.target and args.task == 'adapt':
    # net.module.linear.reset_parameters()
    new_linear = nn.Linear(in_features=net.module.linear.in_features, out_features=net.module.linear.out_features)
    net.module.linear = new_linear.to(device)
    if args.swa:
    #     swa_net.module.linear.reset_parameters()
        new_linear = nn.Linear(in_features=swa_net.module.linear.in_features, out_features=swa_net.module.linear.out_features)
        swa_net.module.linear = new_linear.to(device)
...
if args.target and not args.validate:
    if args.bn_update_first:
        ...
        source_trainset = CIFAR100(root='./data', used_labels=source_labels, used_samples=source_samples,
                           train=True, download=True, transform=transform_train)
...
def test(epoch, swa=False):
    ...
    # assert testnet is net, 'incorrectly use swa model for training. quiting...'
    if testnet is net:
        logger('use the original model for testing')
    elif testnet is swa_net:
        logger('use the swa model for testing')
    testnet.eval()
    test_loss = 0
    correct = 0
    total = 0
    ...
    logger('Epoch {}: Test acc: {}%'.format(epoch, acc))
...
if args.validate:
    ...
    if 'SEMask' in args.arch:
        ...
        C_sparsity = float(numel_C - numel_nnz_C) / numel_C
        C_size = float(numel_nnz_C * C_bit) / 8.0 / 1024.0**2
        En_size = float(numel_C) * 1.0 / 8.0 / 1024.0**2
        B_size = numel_B * 32.0 / 8.0 / 1024.0**2
        model_size = C_size + B_size + En_size
        if args.target and args.task == 'adapt':
            linear_size = net.module.linear.weight.numel() * 32.0 / 8.0 / 1024.0**2
            model_size += linear_size
            logger('\nAll (absolute) values in coefficient matrices include')
            logger('Ce sparsity: {}'.format(C_sparsity))
            logger(sorted(list((vset))))
            logger('model sizes:')
            logger('Ce: {}\tB: {}\tEn: {}\tLinear: {}\tTotal: {}'.format(C_size, B_size, En_size, linear_size, model_size))
        else:
            logger('\nAll (absolute) values in coefficient matrices include')
            logger('Ce sparsity: {}'.format(C_sparsity))
            logger(sorted(list((vset))))
            logger('model sizes:')
            logger('Ce: {}\tB: {}\tEn: {}\tTotal: {}'.format(C_size, B_size, En_size, model_size))
    else:
        i = 0
        sets = []
