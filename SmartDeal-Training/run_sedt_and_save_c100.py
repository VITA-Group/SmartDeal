...
parser.add_argument('--no-quant2', action='store_true', help='Do not quantize Ce to power-of-2 values.')
...
# Must resume a pre-trained model to apply SmartDeal algorithm
...
net = models.__dict__[args.arch](num_classes=100)
net = net.to(device)
se_net = models.__dict__[args.dest_arch](threshold=args.threshold, num_classes=100)
se_net.to(device)
if device == 'cuda':
...
decompose_opts = dict(
    ...
    decompose_quant2 = False if args.no_quant2 else True,
